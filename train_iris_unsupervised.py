...
from args_config import unsuper_args as args_config
...
class DulClsTrainer(object):
    ...
    def __init__(self, args, config):
        ...
        save_to = './checkpoint/Unsuper_'+self.config.data_name+'_'+self.config.test_type+'_'+self.args.fc_mode + '_' + self.args.experiment_name + '_Proj_' +self.args.proj_strategy
    ...
    def _report_settings(self):
        ...
        print("- Problem Setting      : Unsupervised uncertainty contrastive learning")
        self.logger.info("- Problem Setting      : Unsupervised uncertainty contrastive learning")
        ...
        print("- Contrastive projection : {} ".format(self.args.proj_strategy))
        self.logger.info("- Contrastive projection : {} ".format(self.args.proj_strategy))
    ...
    def _train_one_epoch(self, epoch = 0):
        ...
        for idx, (img, gty) in enumerate(self.data['train']):
            ...
            '''
            # Design-1: contrastive loss between two embedding
            detail_loss    = self.model['criterion'](embedding=torch.cat(embedding,dim=0),
                                                     labels=torch.cat((gty, gty),dim=0),
                                                     predy=None,
                                                     mu=mu,
                                                     logvar=logvar)
            
            '''    
            # Design-2: contrastive loss among mu and embedding
            detail_loss    = self.model['criterion'](embedding=(mu, embedding[0], embedding[1]),
                                                     labels=None,
                                                     predy=None,
                                                     mu=mu,
                                                     logvar=logvar)
            ...
            if (idx + 1) % self.args.print_freq == 0:
                print('[UnSUPER] epoch : %2d|%2d, iter : %4d|%4d,  loss : %.4f (%.4f) | contrastive : %.4f   kl : %.4f' % \
                    (epoch, self.args.end_epoch, idx+1, len(self.data['train']), np.mean(loss_recorder), loss.item(), detail_loss['contrastive'].item(),detail_loss['kl'].item()))
                self.logger.info('[UnSUPER] epoch : %2d|%2d, iter : %4d|%4d,  loss : %.4f (%.4f) | contrastive : %.4f   kl : %.4f' % \
                    (epoch, self.args.end_epoch, idx+1, len(self.data['train']), np.mean(loss_recorder), loss.item(), detail_loss['contrastive'].item(),detail_loss['kl'].item()))
    ...
    def _test_model(self,epoch):
        ...
        print('[UnSUPER] EPOCH-{}: EER {:.4f} | R@A1e-1 {:.4f} | R@A1e-3 {:.4f} | R@A1e-5 {:.4f}\n'.format(epoch, eer,prec1,prec2,prec3))
        self.logger.info('[UnSUPER] EPOCH-{}: EER {:.4f} | R@A1e-1 {:.4f} | R@A1e-3 {:.4f} | R@A1e-5 {:.4f}\n'.format(epoch, eer,prec1,prec2,prec3))
